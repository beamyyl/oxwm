...
oxwm.key.bind({ "Control", "Mod1", }, "T", oxwm.spawn_terminal())
-- Launch Dmenu
oxwm.key.bind({ modkey }, "R", oxwm.spawn({ "sh", "-c", "dmenu_run -l 10" }))
-- Copy screenshot to clipboard
oxwm.key.bind({ s }, "Print", oxwm.spawn({ "sh", "-c", "maim -s | xclip -selection clipboard -t image/png" }))
oxwm.key.bind({ "Mod1" }, "F4", oxwm.client.kill()) 

-- Window state toggles
oxwm.key.bind({ modkey }, "F", oxwm.client.toggle_fullscreen())
oxwm.key.bind({ modkey, "Shift" }, "Space", oxwm.client.toggle_floating())

-- Audio (WHY IS THIS NOT HERE BY DEFAULT)
...
-- oxwm.key.bind({}, "XF86AudioRaiseVolume", oxwm.spawn({ "pactl", "set-sink-volume", "@DEFAULT_SINK@", "-5%" }))
-- oxwm.key.bind({}, "XF86AudioRaiseVolume", oxwm.spawn({ "wpctl", "set-sink-mute", "@DEFAULT_SINK@", "toggle" }))
